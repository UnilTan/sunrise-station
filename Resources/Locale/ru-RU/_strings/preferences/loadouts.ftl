# Name
loadout-name-edit-label = Пользовательское имя
loadout-name-edit-label-dataset = Устанавливает пользовательское имя, которое будет использоваться, если вы играете эту роль. Если поле пустое, будет выбрано случайное имя.
loadout-name-edit-tooltip = Максимум 32 символа. Если имя не указано, может быть выбрано случайное имя.
# Restrictions
loadout-restrictions = Ограничения
loadouts-min-limit = Мин. кол-во: { $count }
loadouts-max-limit = Макс. кол-во: { $count }
loadouts-points-limit = Очки: { $count } / { $max }
loadouts-count-items-in-group =
    { $item } and { $count } other { $count ->
        [1] item
       *[other] items
    }
loadouts-points-restriction = Не хватает очков
loadout-group-sex-restriction = Не доступно для данного пола
